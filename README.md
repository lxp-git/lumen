...
Multi-module: `id("io.github.lxp-git.lumen") version "0.2.1" apply false` on the root `plugins {}` block, then `id("io.github.lxp-git.lumen")` on the app module.
...
`id("io.github.lxp-git.lumen")` 0.2.1 is on the Gradle Plugin Portal (first-time namespace approval can take a few days). After that, stock `gradlePluginPortal()` + `mavenCentral()` is enough. The plugin id uses the GitHub namespace because Plugin Portal requires proof of `lumen.dev` for `dev.lumen`.
