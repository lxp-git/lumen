 ...
 * frontend (no extra CSP, no inline-script loader). The legacy Lumen behavior
...
public class ChromeDiscoveryHandler implements HttpHandler {
   ...
   * full Network panel even though the underlying server is Lumen.
   */
  // Keep major version current so Chrome 150+ feature gates don't demote the target.
  private static final String BROWSER_NAME = "Chrome/151.0.7922.71";
   ...
   */
  private static final String V8_VERSION = "13.5.0.0";
   ...
   * frontend wants — but bumping it past lumen's historical "1.3" makes
  ...
  public void register(HandlerRegistry registry) {
    ...
    registry.register(new RegexpPathMatcher(Pattern.compile("/json/close/.*")), this);
  }
  ...
  public boolean handleRequest(SocketLike socket, LightHttpRequest request, LightHttpResponse response) {
    ...
    try {
      ...
      } else if (path != null && path.startsWith("/json/close/")) {
        handleActivate(response);
      } else {
  ...
  private void handleVersion(LightHttpResponse response)
    ...
    if (mVersionResponse == null) {
      ...
      // Chrome treats Browser=Chrome/* as a desktop-like target and stores this
      // URL as browser_target_id. Without it, reconnect HttpUpgrades
      // /devtools/browser, which we historically did not serve.
      reply.put("webSocketDebuggerUrl", "ws://localhost" + mInspectorPath);
   ...
   *       most important difference vs. legacy Lumen and is what lets Chrome
  ...
  private void handlePageList(LightHttpResponse response)
    ...
    if (mPageListResponse == null) {
      ...
      page.put("url", "lumen://" + mContext.getPackageName());
      page.put("webSocketDebuggerUrl", "ws://localhost" + mInspectorPath);
      // Relative favicon keeps chrome://inspect from warning; empty is fine.
      page.put("faviconUrl", "");
      // Explicit bundled frontend URL. Omitting it used to force the bundled UI,
      // but Chrome 150+ sometimes builds a blank tab without this hint when the
      // target is an abstract-socket Android device rather than a WebView.
      page.put(
          "devtoolsFrontendUrl",
          "/devtools/inspector.html?ws=localhost" + mInspectorPath);
      reply.put(page);
