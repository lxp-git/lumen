...
import dev.lumen.dumpapp.DumpappHttpSocketLikeHandler;
import dev.lumen.dumpapp.DumpappSocketLikeHandler;
import dev.lumen.dumpapp.Dumper;
import dev.lumen.dumpapp.DumperPlugin;
import dev.lumen.dumpapp.plugins.CrashDumperPlugin;
import dev.lumen.dumpapp.plugins.FilesDumperPlugin;
import dev.lumen.dumpapp.plugins.HprofDumperPlugin;
import dev.lumen.dumpapp.plugins.SharedPreferencesDumperPlugin;
import dev.lumen.inspector.DevtoolsSocketHandler;
import dev.lumen.inspector.console.RuntimeReplFactory;
import dev.lumen.inspector.database.ContentProviderDatabaseDriver;
import dev.lumen.inspector.database.DatabaseDriver2Adapter;
import dev.lumen.inspector.database.DatabaseFilesProvider;
...
import dev.lumen.inspector.elements.DescriptorProvider;
import dev.lumen.inspector.elements.Document;
import dev.lumen.inspector.elements.DocumentProviderFactory;
...
import dev.lumen.server.LazySocketHandler;
import dev.lumen.server.LocalSocketServer;
import dev.lumen.server.ProtocolDetectingSocketHandler;
import dev.lumen.server.ServerManager;
import dev.lumen.server.SocketHandler;
import dev.lumen.server.SocketHandlerFactory;
...
/**
 * Initialization and configuration entry point for the Lumen debugging system.  Simple usage with
 * default plugins and features enabled:
 * <p />
 * <pre>
 *   Lumen.initializeWithDefaults(context)
 * </pre>
 * <p />
 * For more advanced configuration, see {@link #newInitializerBuilder(Context)} or
 * the {@code lumen-sample} for more information.
 */
public class Lumen {
  private Lumen() {
  }
  ...
  /**
   * Construct a simple initializer helper which allows you to customize lumen behaviour
  ...
  public static void initialize(final Initializer initializer) {
    // Hook activity tracking so that after Lumen is attached we can figure out what
  ...
  /**
   * Convenience mechanism to extend the default set of dumper plugins provided by Lumen.
   *
   * @see #initializeWithDefaults(Context)
   */
  ...
  public static final class DefaultInspectorModulesBuilder {
     ...
     * the existence of {@code lumen-js-rhino} (Mozilla's Rhino engine) and uses it if available.
     * <p />
     * To customize the Rhino implementation, see {@code lumen-js-rhino} documentation.
     */
    ...
    /**
     * Customize the location of database files that Lumen will propogate in the UI.  Android's
    ...
    /**
     * Extend and provide additional database drivers.  Lumen provides two database
     ...
     * </ol>
     *
     * <p>Lumen assumes the {@link SqliteDatabaseDriver} should be installed if
    ...
    public Iterable<ChromeDevtoolsDomain> finish() {
      ...
      // dumpapp-only / legacy callers and skips store-backed Log/Network/Fetch.
  ...
  /**
   * Configure what services are to be enabled in this instance of Lumen.
   */
  public static class InitializerBuilder {
    ...
    /**
     * Enable use of the {@code dumpapp} system.  This is an extension to Lumen which allows
